...
(function($) {
	$.fn.carouseller = function(options) {
		...
		obj = $(this),
		left_btn = obj.find('.carouseller-left'),
		right_btn = obj.find('.carouseller-right'),
		list = obj.find('.carouseller-list'),
		wrapper = obj.find('.carouseller-wrap'),
		count = obj.find('[class*="car-"]').length,
		sens = options.sens,
		...
		get_el_width = function(){
			return obj.find('[class*="car-"]').eq(-1).outerWidth(true);
		},
		...
		left = function(e, result){
			...
			if ( count > can_view) {
				if (e.type == 'touchend') {
					...
					obj.find('[class*="car-"]').eq(-1).prependTo(list);
					list.css({ left: '-'+t+'px' });
					list.stop().animate({
						left: '0px'
					}, options.scrollSpeed, options.easing, function() {
			...
			if ( count > can_view) {
				if (e.type == 'touchend') {
					list.animate({
						...
						obj.find('[class*="car-"]').eq(0).appendTo(list);
						obj.find('[class*="car-"]').eq(0).appendTo(list);
						list.css({ left: '0px' });
